...
Pay bills directly from your business account to a pay bill number or a paybill store.
...
1. The business prepares and sends a payment request to the M-PESA B2B API endpoint with `CommandID: BusinessPayBill`
2. The API Management Platform validates, authorizes, and authenticates the request, forwards it to M-PESA, and sends an acknowledgment to the business
...
- Initiator Username with Org Business Pay Bill API initiator role on M-Pesa
...
| ReferenceData | This JSON object holds more details for the transaction reference data. | JSON Object | `"ReferenceData": {"ReferenceItem": []}` |
| ReferenceItem | A JSON array within that holds JSON Objects with additional transaction details. | JSON Array | `"ReferenceItem": [{"Key": "BillReferenceNumber", "Value": "19008"}]` |
...
Create a test app and select the Business Pay Bill product. The simulator will automatically use your app credentials and test data.
