...
func TestBusinessPayBillForcesCommandID(t *testing.T) {
	token := "test-token-paybill"
	var captured map[string]interface{}
	...
	resp, err := client.BusinessPayBill(context.Background(), req)
	if err != nil {
		t.Fatalf("BusinessPayBill failed: %v", err)
	}
	if resp.ResponseCode != "0" {
		t.Errorf("expected 0, got %s", resp.ResponseCode)
	}
